...
5. ポップアップ内の**マイクボタン**でミュート操作が可能 — Push-to-Talk モードではボタンを長押しして発話
6. Meet タブ上で `Ctrl+Shift+M` でもミュート操作が可能（Push-to-Talk では長押し）
7. Meet タブ上でモード切替の固定ショートカットが使用可能: `Ctrl+Shift+0`（Off）、`Ctrl+Shift+1`（Auto）、`Ctrl+Shift+2`（Auto-Off）、`Ctrl+Shift+3`（Push-to-Talk）

> **注意:** Meet のミュートボタンを直接操作すると、拡張機能は自動的に Off モードに切り替わります。
...
| モードショートカット | Meet タブでモードを切替える固定キー | Ctrl+Shift+0/1/2/3   |
| 発話検出の感度     | 音量がこの値を超えたらミュート解除（モード別） | 0.025                |
| 猶予時間           | 発話終了後、ミュートするまでの待機時間（モード別） | 1.5秒                |
...
- モード別設定: 各モードごとに感度と猶予時間を個別に保存。モード切替時にそのモードの設定が自動復元
